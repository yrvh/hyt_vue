          ...
          <van-checkbox-group v-model="checklist" checked-color="#7EB6FF" ref="select_ref">
            <div v-for="item in list" :key="item.id">
              <van-cell  is-link @click="onListItem(item.id, item.title)"
                        :title="item.goodname" :value="item.fkmoney.replace('&nbsp;&nbsp;','元')">
                <template #label>
                  <div v-text="item.realname + ` (录入: ${item.addtime} )`"></div>
                </template>
              </van-cell>
...
<script>
import {getThirdIncomelist, passUser, nopassUser} from 'network/check'
import {getCoopData, getSellData, getClerkData, getSfidData, getPlatformData} from 'network/common'
...
export default {
  ...
  methods: {
    ...
    onLoad(re_page=false) {   // 加载列表数据==========================================
      ...
      if(this.is_getlist){  // 自动获取列表
        ...
        getThirdIncomelist({...this.param, ...this.obj}).then( res=> {   // 获取列表页数据
  ...
  mounted() {
    this.$axios.all([
      ...
      getPlatformData({...this.obj, ywyid:this.param.ywyid, yxyid:this.param.yxyid, hhrtype:this.hhrtype })
    ]).then(this.$axios.spread((res1,res2,res3,res4,res5) => {
      this.coop_list = res1
      this.sell_list = res2.rows
      this.clerk_list = res3.rows
      this.sfid_list = res4
      this.platform_list = res5
    }))
  },
}
</script>
