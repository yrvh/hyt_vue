...
<script>
import { passUser, nopassUser, getFreeIncomelist} from 'network/check'
...
export default {
  ...
  data() {
    return {
      ...
      yztype_mark: 2,   // 的 tabs标记
      yztype_list: [{text:'全部',id: 2},{text:'有单位业者',id: 0},{text:'无单位业者',id: 1}],   // 业者类型
      sfid_mark: 0,   // 的 tabs标记,,只有当param.addtype==0才有这个
  ...
  methods: {
    ...
    onLoad(re_page=false) {   // 加载列表数据==========================================
      ...
      if(this.is_getlist){  // 自动获取列表
        ...
        getFreeIncomelist({...this.param, ...this.obj}).then( res=> {   // 获取列表页数据
