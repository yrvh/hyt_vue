<template>
  <div class="invoice">
    ...
    <van-tabs type="card" color="#7EB6FF" v-model="tab_mark" animated swipeable scrollspy @change="onChange">
      ...
      <!--    发票查询( 三方平台业者 )      -->
      <van-tab title="三方平台业者" :badge="third_invoice[0].value==0? '':third_invoice[0].value">
        <van-cell v-for="item in third_invoice" :key="item.id" :title="item.title"
                  :value="item.value" :value-class="(item.isright_css && item.value>0)? 'right-css':''"
                  is-link :to="{path: '/check_invoicethirdlist', query: {in_title: item.title, in_status: item.status }}"/>
      </van-tab>
