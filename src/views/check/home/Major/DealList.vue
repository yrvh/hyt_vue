<template>
  <div class="deal-list">
    ...
    <div class="filterbox">
      ...
      <van-tabs v-model="free_mark" color="#7EB6FF" duration="0.3" line-width="15px"
                line-height="5px" title-active-color="#7EB6FF" :change="onChangeFree">
        <van-tab title="业者" disabled/>
        <van-tab v-for="(item,index) in free_list" :key="index" :title="item.name" :name="item.id"/>
      </van-tabs>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      in_title: '',   // 当前进来的标题
      // 筛选条件=================================================
      free_mark: 1,   // 的 tabs标记
      free_list: [{text:'全部',id: 0},{text:'有单位业者',id: 1},{text:'无单位业者',id: 2}],   // 业者类型
  ...
  methods: {
    ...
    onChangeFree(name) {   // 切换了当前 业者类型
      this.param.free = name
      this.onLoad(true)
    },
  ...
  mounted() {
    getFreeData(this.obj).then(res => {
      this.free_list = res
      this.param.free = res[0].id
    })
    this.onLoad()
  },
}
